...
val TrackedDevicePose_t = struct(OPENVR_PACKAGE, "TrackedDevicePose", nativeName = "TrackedDevicePose_t") {
	...
	HmdVector3_t.member("vVelocity", "")
	HmdVector3_t.member("vAngularVelocity", "")
	ETrackingResult.member("eTrackingResult", "")
	bool.member("bPoseIsValid", "")
	bool.member("bDeviceIsConnected", "")
}
...
val VREvent_TouchPadMove_t = struct(OPENVR_PACKAGE, "VREventTouchPadMove", nativeName = "VREvent_TouchPadMove_t") {
	bool.member("bFingerDown", "")
	float.member("flSecondsFingerDown", "")
	float.member("fValueXFirst", "")
	float.member("fValueYFirst", "")
	float.member("fValueXRaw", "")
...
val VRControllerState_t = struct(OPENVR_PACKAGE, "VRControllerState", nativeName = "VRControllerState_t") {
	uint32_t.member("unPacketNum", "")
	uint64_t.member("ulButtonPressed", "")
	uint64_t.member("ulButtonTouched", "")
	VRControllerAxis_t.array("rAxis", "", size = 5)
}
...
val Compositor_FrameTiming = struct(OPENVR_PACKAGE, "CompositorFrameTiming", nativeName = "Compositor_FrameTiming") {
	uint32_t.member("m_nSize", "") // Set to sizeof( Compositor_FrameTiming )
	...
	double.member("m_flSystemTimeInSeconds", "")
	...
	float.member("m_flSubmitFrameMs", "") // time spent in IVRCompositor::Submit (not near-zero indicates driver issue)

	/** The following are all relative to this frame's SystemTimeInSeconds */
	float.member("m_flWaitGetPosesCalledMs", "")
	float.member("m_flNewPosesReadyMs", "")
	float.member("m_flNewFrameReadyMs", "") // second call to IVRCompositor::Submit
...
val RenderModel_t = struct(OPENVR_PACKAGE, "RenderModel", nativeName = "RenderModel_t") {
	...
	// TODO:
	AutoSizeDiv("3", "IndexData")..uint32_t.member("unTriangleCount", "")                    // Number of triangles in the mesh. Index count is 3 * TriangleCount
	TextureID_t.member("diffuseTextureId", "")                // Session unique texture identifier. Rendermodels which share the same texture will have the same id. <0 == texture not present
}
...
val RenderModel_TextureMap_t = struct(OPENVR_PACKAGE, "RenderModelTextureMap", nativeName = "RenderModel_TextureMap_t") {
	...
	const..uint8_t.p.member("rubTextureMapData", "")    // Map texture data. All textures are RGBA with 8 bits per channel per pixel. Data size is width * height * 4ub
}
